...
- 새 module은 `src/modules/example/`을 복사해서 시작한다. `domain → infrastructure → application` 순서로 작성한다. 실제 TypeORM/DB 연동이 필요하면 `src/modules/game/`의 `infrastructure/orm` 패턴을 참고한다.
